...
func (o *GetAssetTokenInfoURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{network}/assets/{asset_id}"

	assetID := o.AssetID
	if assetID != "" {
		_path = strings.Replace(_path, "{asset_id}", assetID, -1)
	} else {
		return nil, errors.New("assetId is required on GetAssetTokenInfoURL")
	}
	...
	if network != "" {
		...
		return nil, errors.New("network is required on GetAssetTokenInfoURL")
	}
...
func (o *GetAssetTokenInfoURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetAssetTokenInfoURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetAssetTokenInfoURL")
	}
