...
func (o *GetAssetsListURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{network}/assets"

	network := o.Network
	if network != "" {
		_path = strings.Replace(_path, "{network}", network, -1)
	} else {
		return nil, errors.New("network is required on GetAssetsListURL")
	}
...
func (o *GetAssetsListURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetAssetsListURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetAssetsListURL")
	}
