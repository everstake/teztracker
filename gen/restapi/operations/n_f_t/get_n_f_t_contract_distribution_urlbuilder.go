...
func (o *GetNFTContractDistributionURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{network}/nft_contracts/{contract_id}/distribution"

	contractID := o.ContractID
	if contractID != "" {
		_path = strings.Replace(_path, "{contract_id}", contractID, -1)
	} else {
		return nil, errors.New("contractId is required on GetNFTContractDistributionURL")
	}
	...
	if network != "" {
		...
		return nil, errors.New("network is required on GetNFTContractDistributionURL")
	}
...
func (o *GetNFTContractDistributionURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetNFTContractDistributionURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetNFTContractDistributionURL")
	}
