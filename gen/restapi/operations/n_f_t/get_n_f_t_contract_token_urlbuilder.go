...
func (o *GetNFTContractTokenURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{network}/nft_contracts/{contract_id}/tokens/{token_id}"

	contractID := o.ContractID
	if contractID != "" {
		_path = strings.Replace(_path, "{contract_id}", contractID, -1)
	} else {
		return nil, errors.New("contractId is required on GetNFTContractTokenURL")
	}
	...
	if network != "" {
		...
		return nil, errors.New("network is required on GetNFTContractTokenURL")
	}
	...
	if tokenID != "" {
		...
		return nil, errors.New("tokenId is required on GetNFTContractTokenURL")
	}
...
func (o *GetNFTContractTokenURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetNFTContractTokenURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetNFTContractTokenURL")
	}
