...
func (o *GetNFTContractTokenHoldersURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{network}/nft_contracts/{contract_id}/tokens/{token_id}/holders"

	contractID := o.ContractID
	if contractID != "" {
		_path = strings.Replace(_path, "{contract_id}", contractID, -1)
	} else {
		return nil, errors.New("contractId is required on GetNFTContractTokenHoldersURL")
	}
	...
	if network != "" {
		...
		return nil, errors.New("network is required on GetNFTContractTokenHoldersURL")
	}

	tokenID := o.TokenID
	if tokenID != "" {
		_path = strings.Replace(_path, "{token_id}", tokenID, -1)
	} else {
		return nil, errors.New("tokenId is required on GetNFTContractTokenHoldersURL")
	}
...
func (o *GetNFTContractTokenHoldersURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetNFTContractTokenHoldersURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetNFTContractTokenHoldersURL")
	}
