...
func (o *GetLostBlocksAggCountURL) WithBasePath(bp string) *GetLostBlocksAggCountURL {
	o.SetBasePath(bp)
	return o
}
...
func (o *GetLostBlocksAggCountURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{platform}/{network}/lost/blocks/agg"

	network := o.Network
	if network != "" {
		_path = strings.Replace(_path, "{network}", network, -1)
	} else {
		return nil, errors.New("network is required on GetLostBlocksAggCountURL")
	}
	...
	if platform != "" {
		...
		return nil, errors.New("platform is required on GetLostBlocksAggCountURL")
	}
...
func (o *GetLostBlocksAggCountURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetLostBlocksAggCountURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetLostBlocksAggCountURL")
	}
