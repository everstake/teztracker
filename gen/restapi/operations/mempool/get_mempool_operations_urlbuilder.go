...
type GetMempoolOperationsURL struct {
	Network  string
	Platform string
...
func (o *GetMempoolOperationsURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{platform}/{network}/mempool"

	network := o.Network
	if network != "" {
		_path = strings.Replace(_path, "{network}", network, -1)
	} else {
		return nil, errors.New("network is required on GetMempoolOperationsURL")
	}
	...
	if platform != "" {
		...
		return nil, errors.New("platform is required on GetMempoolOperationsURL")
	}
...
func (o *GetMempoolOperationsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetMempoolOperationsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetMempoolOperationsURL")
	}
