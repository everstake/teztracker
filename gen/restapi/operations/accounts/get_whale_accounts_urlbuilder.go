...
func (o *GetWhaleAccountsURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{platform}/{network}/whale/accounts"

	network := o.Network
	if network != "" {
		_path = strings.Replace(_path, "{network}", network, -1)
	} else {
		return nil, errors.New("network is required on GetWhaleAccountsURL")
	}
	...
	if platform != "" {
		...
		return nil, errors.New("platform is required on GetWhaleAccountsURL")
	}
...
func (o *GetWhaleAccountsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetWhaleAccountsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetWhaleAccountsURL")
	}
