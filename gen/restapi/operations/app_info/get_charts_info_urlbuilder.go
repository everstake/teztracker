...
func (o *GetChartsInfoURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{platform}/{network}/charts"

	network := o.Network
	if network != "" {
		_path = strings.Replace(_path, "{network}", network, -1)
	} else {
		return nil, errors.New("network is required on GetChartsInfoURL")
	}
	...
	if platform != "" {
		...
		return nil, errors.New("platform is required on GetChartsInfoURL")
	}
	...
	for _, columnsI := range o.Columns {
		columnsIS := columnsI
		if columnsIS != "" {
			columnsIR = append(columnsIR, columnsIS)
		}
	}

	columns := swag.JoinByFormat(columnsIR, "")
...
func (o *GetChartsInfoURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetChartsInfoURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetChartsInfoURL")
	}
