...
func (o *GetBakerChartInfoURL) Build() (*url.URL, error) {
	...
	var _path = "/v2/data/{platform}/{network}/charts/bakers"

	network := o.Network
	if network != "" {
		_path = strings.Replace(_path, "{network}", network, -1)
	} else {
		return nil, errors.New("network is required on GetBakerChartInfoURL")
	}
	...
	if platform != "" {
		...
		return nil, errors.New("platform is required on GetBakerChartInfoURL")
	}
...
func (o *GetBakerChartInfoURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetBakerChartInfoURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetBakerChartInfoURL")
	}
