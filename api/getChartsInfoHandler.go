...
func (h *getChartsInfoHandler) Handle(params info.GetChartsInfoParams) middleware.Responder {
	...
	for key := range columns {
		if err := validate.Enum(fmt.Sprintf("%s.%v", "column", key), "query", columns[key], []interface{}{"blocks", "volume", "operations", "avg_block_delay", "fees", "activations", "delegation_volume", "bakers", "blocks_priority"}); err != nil {
			return info.NewGetChartsInfoBadRequest()
		}
	}
