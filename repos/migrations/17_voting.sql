...
select period, proposal, source, rolls, kind, s.block_level as block_level
from (select period,  unnest(regexp_split_to_array(replace(replace(proposal :: text, '[', ''), ']', ''), ',')) as proposal, kind, source, min(block_level) as block_level
...
select sum(rolls) as rolls, count(1) as bakers, min(block_level)  as block_level, proposal, period, kind
from tezos.voting_view
group by proposal, period, kind;
